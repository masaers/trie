...
namespace com_masaers {
  ...
  template<typename Element,
	   typename Comp = std::less<Element> >
  using ordered_trie_set = basic_trie_set<ordered_trie_node_t<Element, Comp> >;

  template<typename Element,
	   typename Hash = std::hash<Element>,
	   typename Equal = std::equal_to<Element> >
  using unordered_trie_set = basic_trie_set<unordered_trie_node_t<Element, Hash, Equal> >;
